...
$script = '
    $(document).ready(
        function()
        {
				...
				var Request = $.ajax(
					{
						...
						data: { article_id: $("#hidden_article_id").val(), qte: $("#qte").val(), status: "'.$this->request->data['Facture']['statut_id'].'" },
						cache: false,
						type: "POST",
						dataType: "json"
					}
            ...
            $("form#FactureEditForm").validationEngine();
            $("#FactureClientId").chosen();
            $("#FactureArticleAticleId").chosen();
            $("#FacturePaiementId").chosen();
            $("#FactureTaxId").chosen();
            $("#FactureStatutId").chosen();
                        ...
                        function(json, status, jqXHR)
                        {
                            if (json.check == 0)
                            {
                                $("#getcontent").fadeIn(
                                    ...
                                    {
                                        $("#getcontent").load(
                                            "'.$this->Html->url(array('action' => 'getfacture', $this->request->params['pass'][0])).'",
                                            function()
                                            {
                                                $.fancybox.close();
    ...
    function validateStep2()
    {
        ...
        if (($("#FactureReference").val() != "") && ($("#FactureDate").val() != "") && ($("#FactureName").val() != "")) {
            isValid = true;
        }
        return isValid;
    }
...
<div>
    <h2>
        FORMULAIRE 
        <?php
        if ((int)$this->request->data['Facture']['statut_id'] === (int) Configure::read('bCloud.Facture.Statut.FACTURE')) {
            echo 'FACTURE';
        } else {
            echo 'PROFORMA';
        }
        ...
        <div id="step-1" style="width:100%; height: auto">
            ...
            <div class="section">
                ...
                <div>
                    ...
                    <?php echo $this->Form->input('status', array('type' => 'hidden', 'value' => $this->request->data['Facture']['statut_id'])); ?>
