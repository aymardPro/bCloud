...
<table class="table table-bordered table-striped bcloud">
	<thead>
		<tr>
			<th class="left"> <?php echo __('MODALITE DE PAIEMENT') ?> </th>
	...
	<tbody align="center">
		...
		<tr class="odd gradeX">
			<td class="left"> <?php echo h($data['PaiementModalite']['name']); ?> </td>
			<td>
				<?php
				echo $this->Form->postLink(
    				$this->Html->image('/images/icon/coquette-icons-set/24x24/delete.png'),
    				array('action' => 'delete', $data['PaiementModalite']['id']),
    				array('escape' => false, 'confirm' => __('Voulez-vous supprimer définitivement %s ?', $data['PaiementModalite']['name']))
    			);
				?>
			</td>
		</tr>
