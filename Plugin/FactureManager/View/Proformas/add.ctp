...
$script = '
    $(document).ready(
        function()
        {
			...
			$("#ProformaRemise").mask("99%",{completed:function(){ Processgraph(); }});
			$("#qte").spinner({min: 1, stepping: 1, decimals:0});
			
			$("#set_form").click(function(e) {
				alertHide();
			    loading("Loading");
				$("#set_form").attr("disabled", "disabled").html("Ajout en cours <img src=\"./images/loadder/loader_green.gif\" />");
                ...
                Request.done(function(json, status, jqXHR)
                   	{
                   		if (json.check > 0) {
							...
							$("#getArticle").load(
								"'.$this->Html->url(array('action' => 'getarticle')).'",
								function()
								{
									unloading();
            ...
            $("form#ProformaAddForm").validationEngine();
            $("select").chosen();
            $("#ProformaDate").datepicker({
                dateFormat:"yy-mm-dd",
                appendText: "Format: aaaa-mm-jj"
            });
            $("#ProformaEcheance").datepicker({
                dateFormat:"yy-mm-dd",
                appendText: "Format: aaaa-mm-jj"
            });
