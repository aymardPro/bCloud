...
<h2>Liste des mouvements d'entrée</h2>
<hr />
...
<table class="table table-striped bcloud">
	<thead>
		<tr>
			<th width="5%"> &nbsp; </th>
            <th class="left" width="15%"> <?php echo __('CODE') ?> </th>
            <th class="left"> <?php echo __('INTITULE') ?> </th>
            <th class="left"> <?php echo __('DEPOT') ?> </th>
	...
	<tbody align="center">
		...
		<tr class="odd gradeX">
			...
			<td class="left">
			    ...
			    echo __(
                    '%s%s',
                    $this->requestAction(array('action' => 'getprep', $data['Mouvement']['type'])),
                    $this->requestAction(array('action' => 'getcode', $data['Mouvement']['id']))
                );
                ?>
			</td>
            <td class="left">
                <?php echo h($data['Mouvement']['name']); ?>
            </td>
            <td class="left">
                <?php echo h($data['Depot']['name']); ?>
            </td>
            <td class="center">
                <?php echo CakeTime::format($data['Mouvement']['date'], '%d-%m-%Y'); ?>
            </td>
			<td>
				<?php
